...
START_TEST(test_nth){
  ...
  ck_assert_ptr_eq(nth(head, 0), &item1);
  ck_assert_ptr_eq(nth(head, 1), &item2);
  ck_assert_ptr_eq(nth(head, 2), &item3);
  ck_assert_ptr_eq(nth(head, 3), &item4);
  ck_assert_ptr_null(nth(head, 4));
  ck_assert_ptr_null(nth(head, 1000));
  ck_assert_ptr_null(nth(head, -1));
}
END_TEST
...
Suite *make_func_suite(void)
{
  ...
  TCase *tc_nth = tcase_create("nth");
  suite_add_tcase(s, tc_nth);
  tcase_add_checked_fixture(tc_nth, setup, teardown);
  {
    tcase_add_test(tc_nth, test_nth);
  }
